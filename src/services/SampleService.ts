...
export const insertDB = async (sample: any) =>
    ...
    await lock.acquire('insertDB', async (done) => {
      try {
        ...
        db.updateLastSampleEndTime(sample.timestamp).then(async () => {
          ...
          const finalSample = {
            ...sampleObj,
            hash: sha256(JSON.stringify(sampleObj)),
          };

          await db.addSample(finalSample);

          const isExist = await wifiMacAddressDatabase.containsWifiHash(
            wifiHash
          );

          if (!isExist) {
            await wifiMacAddressDatabase.addWifiMacAddresses({
              wifiHash,
              wifiList,
            });
          }
...
const hasFirstPointTimestamp = () =>
  new Promise(async (resolve) => {
    try {
      resolve(
        JSON.parse((await AsyncStorage.getItem(FIRST_POINT_TS)) || 'false')
      );
    } catch (error) {
      resolve(false);
      onError({ error });
...
const hasLastPointTimestamp = () =>
  new Promise(async (resolve) => {
    try {
      resolve(
        JSON.parse(
          (await AsyncStorage.getItem(LAST_POINT_START_TIME)) || 'false'
        )
      );
    } catch (error) {
      resolve(false);

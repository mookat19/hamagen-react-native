...
export const toggleChangeLanguage = (isShow: boolean) => (dispatch: any) =>
  dispatch({ type: TOGGLE_CHANGE_LANGUAGE, payload: isShow });
...
export const initLocale = () => async (dispatch: any) => {
  try {
    ...
    let activeLocale: 'he' | 'iw' | 'en' | 'ar' | 'am' | 'ru' = (
      (await AsyncStorage.getItem(CURRENT_LOCALE)) || locale
    ).substr(0, 2);
    ...
    const { data } = await axios.get(
      `${config().stringsUrl}?r=${Math.random()}`,
    ...
    dispatch({
      type: LOCALE_CHANGED,
      payload: { strings: LocaleData.he, locale: 'he', isRTL: true },
    });
    onError({ error });
  }
};

...
export const setExposures = (exposures: Exposure[]) => async (
  dispatch: any
) => {
  ...
  if (dismissedExposures) {
    ...
    filteredExposures = exposures.filter(
      (exposure) =>
        !parsedDismissedExposures.includes(exposure.properties.OBJECTID)
    );
  }

  dispatch({
    type: UPDATE_EXPOSURES,
    payload: { exposures: filteredExposures || exposures },
  });
  dispatch({
    type: UPDATE_PAST_EXPOSURES,
    payload: { pastExposures: exposures },
  });
};

export const setValidExposure = (exposure: Exposure) => async (
  dispatch: any
) => {
  try {
    await AsyncStorage.setItem(
      VALID_EXPOSURE,
      JSON.stringify({ exposure, timestamp: moment().valueOf() })
    );
    dispatch({
...
export const dismissExposure = (exposureId: number) => async (
  dispatch: any
) => {
  ...
  if (dismissedExposures) {
    ...
    await AsyncStorage.setItem(
      DISMISSED_EXPOSURES,
      JSON.stringify(parsedDismissedExposures)
    );
  } else {
    await AsyncStorage.setItem(
      DISMISSED_EXPOSURES,
      JSON.stringify([exposureId])
    );
  }
};

...
export const toggleLoader = (isShow: boolean) => (dispatch: any) =>
  dispatch({ type: TOGGLE_LOADER, payload: { isShow } });

export const toggleWebview = (isShow: boolean, usageType: string) => (
  dispatch: any
) => dispatch({ type: TOGGLE_WEBVIEW, payload: { isShow, usageType } });

export const checkForceUpdate = () => async (dispatch: any) => {
  try {
    const {
      data: { ios, android, shouldForceIOS, shouldForceAndroid, terms },
    } = await axios.get(`${config().versionsUrl}?r=${Math.random()}`, {
      headers: { 'Content-Type': 'application/json;charset=utf-8' },
    });

    const termsVersion = JSON.parse(
      (await AsyncStorage.getItem(CURRENT_TERMS_VERSION)) || '0'
    );
